...
export default function Page() {
  ...
  const isStoriesList = pathname === "/Stories";
  const [currentSlide, setCurrentSlide] = useState(0);
  const [activeSlides, setActiveSlides] = useState([]);
  const [isFading, setIsFading] = useState(false);
  const [sortOrder, setSortOrder] = useState("asc");
  const isStoriesPage = pathname.startsWith("/Stories");
  const isStoryDetail = isStoriesPage && !!selectedStory;
  ...
  const handleProductClick = (product) => {
    ...
    const allImages = Slide[0].images;

    const reorderedImages = [
      product.img,
      ...allImages.filter((img) => img !== product.img),
    ];

    setActiveSlides(reorderedImages);
    setCurrentSlide(0);
  ...
  const showDesktopZoomBar = !isMobile && !isStoryDetail && !selectedShoe;

  return (
    <>
      <div className="relative min-h-screen h-[500px] ">
        {selectedStory && !selectedShoe && (
          <div className="max-w-5xl mx-auto pt-10 mb-10">
